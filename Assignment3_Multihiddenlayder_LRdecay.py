...
with graph.as_default():
  ...
  h=tf.nn.relu(tf.matmul(tf_train_dataset,w1)+b1)
  
  keep_prob = tf.placeholder(tf.float32) #prob 3
  h_drop=tf.nn.dropout(h,keep_prob)

  l2reg=tf.nn.l2_loss(w1)+tf.nn.l2_loss(b1) #initialize l2reg
  ...
  for layer in range(n_hlayer-1):
       ...
       if layer==0:
           ...
           w3=w
           b3=b
        #add more here if more than 3 hidden layers
  ...
  #with decayed learning rate:
#  global_step=tf.Variable(0)
#  learning_rate=tf.train.exponential_decay(.5,global_step,500,.9,staircase=True)
#  optimizer = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss,global_step=global_step)
  ...
  for layer in range(n_hlayer):
      ...
      #add more if there are more than 3 hidden layers   
...
# Prob4: a. test accuracy with 2 hidden layers (1024 and 512): 94.6% with beta = 0 
# Prob4: b. add learning rate decay 
